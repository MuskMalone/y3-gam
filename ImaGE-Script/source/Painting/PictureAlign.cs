...
public class PictureAlign : Entity
{
    ...
    bool IsAligned()
    {
        ...
        float rotationDifference = Quaternion.Angle(player.rotation, savedRotation);
        ...
        {
            Debug.Log(Vector3.Distance(player.position, savedPosition));
            Debug.Log("Player position not aligned.");
            return false;
        }

        // Check if the player's rotation is within the threshold of the saved rotation
        if (Quaternion.Angle(player.rotation, savedRotation) > rotationThreshold)
        {
        ...
        if (Quaternion.Angle(mainCamera.transform.rotation, savedCameraRotation) > rotationThreshold)
        {
    ...
    //    string dataPath = Path.Combine(Application.persistentDataPath, dataFileName);
