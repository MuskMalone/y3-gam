...
uniform sampler2D[16] u_NormalMaps;  // Normal maps array
...
uniform int u_type[maxLights];       // Light type array
...
void main() {
    ...
    // Calculate texture coordinates for both albedo and normal maps.
    ...
    // Check for transparency and discard the fragment if nearly transparent.
    if (albedoTexture.a * mat.AlbedoColor.a < 0.01) {
        discard;
    }

    // Start with the interpolated vertex normal.
    vec3 N = normalize(v_Normal);

    // Sample the normal map.
    vec3 normalSample = texture2D(u_NormalMaps[int(v_MaterialIdx) + u_MatIdxOffset], texCoord).rgb;
    // If the normal map is not just white (default), adjust the normal.
    if (length(normalSample - vec3(1.0)) > 0.001) {
        // Convert from [0,1] to [-1,1].
        vec3 tangentNormal = normalSample * 2.0 - 1.0;
        // Build the TBN matrix from the vertex tangent, bitangent, and normal.
        mat3 TBN = mat3(normalize(v_Tangent), normalize(v_Bitangent), normalize(v_Normal));
        // Transform the tangent-space normal to world space.
        N = normalize(TBN * tangentNormal);
    }

    // Continue with the PBR calculations using the (potentially modified) normal N.
    ...
    for (int i = 0; i < numlights; ++i) {
        ...
        if (u_type[i] == typeDir) {
            if (i > 0) // Use only one directional light
                continue;
            l = -u_LightDirection[i];
        ...
        // Use the modified normal N for lighting calculations.
        vec3 n = N;
        vec3 h = normalize(V + l);

        // Half-Lambert shading for diffuse.
        ...
        vec3 DiffuseBRDF = kD * fLambert / PI * NdotL;
    ...
    float luminance = dot(mat.Emission.xyz, vec3(0.2126, 0.7152, 0.0722));

    bloomColor = vec4(0, 0, 0, 1);
    if (v_BloomProps.x > 0.1) {
