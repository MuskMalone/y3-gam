...
vec3 get(vec2 uv, float off, float d, float yscale) {
    float offd = off * d;
    return texture(u_ScreenTex, uv + vec2(offd, yscale * offd)).xyz;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    /*
    fragColor = vec4(texture(u_ScreenTex, uv).xyz, 1.0);
    return;
    */
    uv -= vec2(0.5);
    uv = fisheye(uv);
    ...
    uv += vec2(0.5);
    float mframe = float(u_Frame % 2);
    uv.y += mframe * 1.0 / YRES * SCANFLICKER;
    
    // interference
    
    float r = random(vec2(0.0, scany), u_Time);
    if (r > 0.995) {r *= 3.0;}
    float ifx1 = INTERFERENCE1 * 2.0 / u_Resolution.x * r;
    ...
    vec3 sat = texture(u_ScreenTex, uv).xyz;
    ...
    float chroma_phase = u_Time * 60.0 * PI * 0.6667;
