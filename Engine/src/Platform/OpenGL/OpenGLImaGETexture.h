...
class OpenGLImaGETexture : public ImaGETexture
{
public:
	OpenGLImaGETexture();
	OpenGLImaGETexture(const char* fileLoc, bool flipVert = false, bool isSampler = false, int filter = 0);
	OpenGLImaGETexture(const char* fileLoc, uint32_t width, uint32_t height, bool isSampler, int filter);
	OpenGLImaGETexture(const char* fileLoc, Specification spec); // constructor for fully customizable texture
	virtual ~OpenGLImaGETexture();
