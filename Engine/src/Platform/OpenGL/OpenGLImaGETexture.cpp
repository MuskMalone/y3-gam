...
OpenGLImaGETexture::OpenGLImaGETexture()
{
	m_Spec.Width = 0;
	m_Spec.Height = 0;
	m_Spec.BitDepth = 0;
...
OpenGLImaGETexture::OpenGLImaGETexture(const char* fileLoc, bool flipVert, bool isSampler, int filter)
	: OpenGLImaGETexture()
{
...
OpenGLImaGETexture::OpenGLImaGETexture(const char* fileLoc, uint32_t width, uint32_t height, bool isSampler, int filter)
	: OpenGLImaGETexture()
{
	m_FileLocation = fileLoc;
	m_Spec.Width = width;
...
*/
OpenGLImaGETexture::OpenGLImaGETexture(const char* fileLoc, Specification spec)
	: OpenGLImaGETexture()
{
	m_FileLocation = fileLoc;
...
bool OpenGLImaGETexture::Load(bool flipVert)
{
	if (m_Buffer)
	{
...
void OpenGLImaGETexture::CreateAPISpecific()
{
...
void OpenGLImaGETexture::SetRed(int x, int z, int value)
{
	m_Buffer[((z * m_Spec.Width + x) * m_Spec.BitDepth) + 0] = value;
}

void OpenGLImaGETexture::SetGreen(int x, int z, int value)
{
	m_Buffer[((z * m_Spec.Width + x) * m_Spec.BitDepth) + 1] = value;
}

void OpenGLImaGETexture::SetBlue(int x, int z, int value)
{
	m_Buffer[((z * m_Spec.Width + x) * m_Spec.BitDepth) + 2] = value;
}

void OpenGLImaGETexture::SetAlpha(int x, int z, int value)
{
	m_Buffer[((z * m_Spec.Width + x) * m_Spec.BitDepth) + 3] = value;
}

void OpenGLImaGETexture::Bind(uint32_t textureSlot) const
{
	glActiveTexture(GL_TEXTURE0 + textureSlot);
	glBindTexture(GL_TEXTURE_2D, m_ID);
}

void OpenGLImaGETexture::Unbind()
{
	glBindTexture(GL_TEXTURE_2D, 0);
}

float OpenGLImaGETexture::GetFileSize(const char* filename)
{
...
uint32_t OpenGLImaGETexture::CalculateMipMapCount(uint32_t width, uint32_t height)
{
	uint32_t levels = 1;
...
uint32_t OpenGLImaGETexture::GetMipLevelCount()
{
	return CalculateMipMapCount(m_Spec.Width, m_Spec.Height);
}

std::pair<uint32_t, uint32_t> OpenGLImaGETexture::GetMipSize(uint32_t mip) const
{
	Log::GetLogger()->error("OpenGLImaGETexture::GetMipSize({0}) - method not implemented!", mip);
