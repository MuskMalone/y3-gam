...
std::map<std::string, H2M::RefH2M<ImaGETexture>> ResourceManager::s_Textures;
...
std::map<std::string, H2M::RefH2M<ImaGEShader>> ResourceManager::s_ShaderCacheByTitle;

std::map<std::string, H2M::RefH2M<ImaGEShader>> ResourceManager::s_ShadersCacheByFilepath;
...
H2M::RefH2M<ImaGETexture> ResourceManager::HotLoadTexture(std::string textureName)
{
...
void ResourceManager::AddShader(std::string name, H2M::RefH2M<ImaGEShader> shader)
{
...
const H2M::RefH2M<ImaGEShader>& ResourceManager::GetShader(std::string name)
{
    if (s_ShaderCacheByTitle.find(name) != s_ShaderCacheByTitle.end()) {
...
const H2M::RefH2M<ImaGEShader>& ResourceManager::CreateOrLoadShader(ImaGEShaderSpecification imaGEShaderSpecification)
{
    H2M::RefH2M<ImaGEShader> imaGEShader;

    std::string pixelShaderPath = imaGEShaderSpecification.PixelShaderPath;
    std::map<std::string, H2M::RefH2M<ImaGEShader>>::iterator entry = s_ShadersCacheByFilepath.find(pixelShaderPath);
    if (entry != s_ShadersCacheByFilepath.end()) {
        // A cache HIT
        imaGEShader = entry->second;
        Log::GetLogger()->info("ResourceManager: A shader loaded from the cache [key: '{0}']", pixelShaderPath);
    }
    else {
        // A cache MISS
        imaGEShader = ImaGEShader::Create(imaGEShaderSpecification);
        s_ShadersCacheByFilepath.insert(std::make_pair(pixelShaderPath, imaGEShader));
