...
H2M::RefH2M<ImaGETexture> TextureLoader::GetTexture(const char* fileLoc, bool flipVert, bool force)
{
	return GetTexture(fileLoc, flipVert, GL_LINEAR, force);
}

H2M::RefH2M<ImaGETexture> TextureLoader::GetTexture(const char* fileLoc, bool flipVert, GLenum filter, bool force)
{
	if (!force) {
		std::map<std::string, H2M::RefH2M<ImaGETexture>>::iterator it;
	...
	H2M::RefH2M<ImaGETexture> texture = ImaGETexture::Create(fileLoc, false, true, filter);
...
{
	std::map<std::string, H2M::RefH2M<ImaGETexture>>::reverse_iterator rit;
