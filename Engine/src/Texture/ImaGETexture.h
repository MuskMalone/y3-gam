...
class ImaGETexture : public H2M::Texture2D_H2M
{
public:
...
public:
	static H2M::RefH2M<ImaGETexture> Create(const char* fileLoc, bool flipVert = false, bool isSampler = false, int filter = 0);
	static H2M::RefH2M<ImaGETexture> Create(const char* fileLoc, uint32_t width, uint32_t height, bool isSampler, int filter);
	static H2M::RefH2M<ImaGETexture> Create(const char* fileLoc, Specification spec);
