...
H2M::RefH2M<ImaGEShader> EnvMapSceneRenderer::s_ShaderEquirectangularConversion;
H2M::RefH2M<ImaGEShader> EnvMapSceneRenderer::s_ShaderEnvFiltering;
H2M::RefH2M<ImaGEShader> EnvMapSceneRenderer::s_ShaderEnvIrradiance;
H2M::RefH2M<ImaGEShader> EnvMapSceneRenderer::s_ShaderGrid;
H2M::RefH2M<ImaGEShader> EnvMapSceneRenderer::s_ShaderSkybox;
...
struct EnvMapSceneRendererData
{
    ...
    H2M::RefH2M<ImaGEShader> CompositeShader;
...
void EnvMapSceneRenderer::Init(std::string filepath, H2M::SceneH2M* scene, EnvMapEditorLayer* editorLayer)
{
    ...
    geoRenderPassSpec.TargetFramebuffer = ImaGEFramebuffer::Create(geoFramebufferSpec);

    auto targetFramebufferGeo = static_cast<H2M::RefH2M<ImaGEFramebuffer>>(geoRenderPassSpec.TargetFramebuffer);
    ...
    compRenderPassSpec.TargetFramebuffer = ImaGEFramebuffer::Create(compFramebufferSpec);

    auto targetFramebufferComp = static_cast<H2M::RefH2M<ImaGEFramebuffer>>(compRenderPassSpec.TargetFramebuffer);
...
{
    ImaGEShaderSpecification imaGEShaderSpecification;
    imaGEShaderSpecification.ShaderType = ImaGEShaderSpecification::ShaderType::ImaGEShader;
    imaGEShaderSpecification.VertexShaderPath = "Shaders/Hazel/SceneComposite.vs";
    imaGEShaderSpecification.FragmentShaderPath = "Shaders/Hazel/SceneComposite.fs";
    imaGEShaderSpecification.ForceCompile = false;
    s_Data.CompositeShader = ImaGEShader::Create(imaGEShaderSpecification);
    Log::GetLogger()->info("EnvMapSceneRenderer: s_ShaderComposite compiled [programID={0}]", s_Data.CompositeShader->GetProgramID());

    s_ShaderEquirectangularConversion = ImaGEShader::Create("Shaders/Hazel/EquirectangularToCubeMap.cs");
...
void EnvMapSceneRenderer::RenderOutline(H2M::RefH2M<ImaGEShader> shader, H2M::EntityH2M entity, const glm::mat4& entityTransform, H2M::RefH2M<H2M::SubmeshH2M> submesh)
{
...
void EnvMapSceneRenderer::UpdateShaderPBRUniforms(H2M::RefH2M<ImaGEShader> shaderHazelPBR, H2M::RefH2M<EnvMapMaterial> envMapMaterial)
{
    ...
    if (meshEntities.size())
    {
        for (auto entt : meshEntities)
        {
            ...
            if (meshComponent.Mesh)
            {
                ...
                EnvMapSharedData::s_ShaderHazelPBR = meshComponent.Mesh->IsAnimated() ? ImaGEShaderLibrary::Get("HazelPBR_Anim") : ImaGEShaderLibrary::Get("HazelPBR_Static");
...
void EnvMapSceneRenderer::CompositePass()
{
    ...
    auto targetFramebuffer = static_cast<H2M::RefH2M<ImaGEFramebuffer>>(s_Data.GeoPass->GetSpecification().TargetFramebuffer);
    ...
    // ERROR: OpenGLImaGEShader::SetFloat() failed [name='u_Exposure', location='-1']
...
{
    auto targetFramebuffer = static_cast<H2M::RefH2M<ImaGEFramebuffer>>(s_Data.CompositePass->GetSpecification().TargetFramebuffer);
...
H2M::RefH2M<ImaGEShader> EnvMapSceneRenderer::GetShaderComposite()
{
