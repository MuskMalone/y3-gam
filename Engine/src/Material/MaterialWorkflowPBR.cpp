...
void MaterialWorkflowPBR::SetupShaders()
{
	...
	{
		m_ShaderEquirectangularToCubemap = H2M::RefH2M<OpenGLImaGEShader>(ImaGEShader::Create("Shaders/PBR/cubemap.vs", "Shaders/PBR/equirectangular_to_cubemap.fs"));
	...
	{
		m_ShaderIrradiance = H2M::RefH2M<OpenGLImaGEShader>(ImaGEShader::Create("Shaders/PBR/cubemap.vs", "Shaders/PBR/irradiance_convolution.fs"));
	...
	{
		m_ShaderPrefilter = H2M::RefH2M<OpenGLImaGEShader>(ImaGEShader::Create("Shaders/PBR/cubemap.vs", "Shaders/PBR/prefilter.fs"));
		Log::GetLogger()->info("MaterialWorkflowPBR: m_ShaderPrefilter compiled [programID={0}]", m_ShaderPrefilter->GetProgramID());
	}

	if (!m_ShaderBRDF)
	{
		m_ShaderBRDF = H2M::RefH2M<OpenGLImaGEShader>(ImaGEShader::Create("Shaders/PBR/brdf.vs", "Shaders/PBR/brdf.fs"));
	...
	{
		m_ShaderHorizontalBlur = H2M::RefH2M<OpenGLImaGEShader>(ImaGEShader::Create("Shaders/ThinMatrix/blur_horizontal.vs", "Shaders/ThinMatrix/blur.fs"));
	...
	{
		m_ShaderVerticalBlur = H2M::RefH2M<OpenGLImaGEShader>(ImaGEShader::Create("Shaders/ThinMatrix/blur_vertical.vs", "Shaders/ThinMatrix/blur.fs"));
