...
{
    m_ShaderMain = ImaGEShader::Create("Shaders/shader.vert", "Shaders/shader.frag");
    Log::GetLogger()->info("SceneAnimPBR: m_ShaderMain compiled [programID={0}]", m_ShaderMain->GetProgramID());

    m_ShaderBackground = ImaGEShader::Create("Shaders/LearnOpenGL/2.2.2.background.vs", "Shaders/LearnOpenGL/2.2.2.background.fs");
    Log::GetLogger()->info("SceneAnimPBR: m_ShaderBackground compiled [programID={0}]", m_ShaderBackground->GetProgramID());

    m_ShaderHybridAnimPBR = ImaGEShader::Create("Shaders/HybridAnimPBR.vs", "Shaders/HybridAnimPBR.fs");
    Log::GetLogger()->info("SceneAnimPBR: m_ShaderHybridAnimPBR compiled [programID={0}]", m_ShaderHybridAnimPBR->GetProgramID());

    m_ShaderEquirectangularConversion = ImaGEShader::Create("Shaders/Hazel/EquirectangularToCubeMap.cs");
    ...
    m_ShaderEnvFiltering = ImaGEShader::Create("Shaders/Hazel/EnvironmentMipFilter.cs");
    Log::GetLogger()->info("SceneAnimPBR: m_ShaderEnvFiltering compiled [programID={0}]", m_ShaderEnvFiltering->GetProgramID());

    m_ShaderEnvIrradiance = ImaGEShader::Create("Shaders/Hazel/EnvironmentIrradiance.cs");
    Log::GetLogger()->info("SceneAnimPBR: m_ShaderEnvIrradiance compiled [programID={0}]", m_ShaderEnvIrradiance->GetProgramID());

    m_ShaderBasic = ImaGEShader::Create("Shaders/basic.vs", "Shaders/basic.fs");
