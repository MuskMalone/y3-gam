...
void SceneEditor::LoadScene()
{
    ...
    std::string sceneFileContent = ImaGEShader::ReadFile(m_SceneFilename);
    ...
    if (m_ShowWindowTextures)
    {
        ImGui::Begin("Textures", &m_ShowWindowTextures);
        {
            if (ImGui::CollapsingHeader("Display Info"))
            {
                ...
                for (std::map<std::string, H2M::RefH2M<ImaGETexture>>::iterator it = textures.begin(); it != textures.end(); ++it)
                {
                ...
                if (ImGui::CollapsingHeader("Normals", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
                {
                    ...
                    if (ImGui::IsItemHovered())
                    {
                        ...
                        if (ImGui::IsItemClicked())
                        {
                            ...
                            if (filename != "")
                                m_LoadedTexture = ImaGETexture::Create(filename.c_str(), false);
                        }
                    }
                    ImGui::SameLine();
...
void SceneEditor::RenderImGuiMenu(Window* mainWindow, ImGuiDockNodeFlags dockspaceFlags)
{
...
Mesh* SceneEditor::CreateNewMesh(int meshTypeID, glm::vec3 scale, std::string* name)
{
    ...
    case MESH_TYPE_M1911:
        mesh = new H2M::MeshH2M("Models/M1911/m1911.fbx", H2M::RefH2M<ImaGEShader>(RendererBasic::GetShaders()["hybrid_anim_pbr"]), (*ResourceManager::GetMaterials())["M1911"], true);
        *name = "M1911";
        break;
    default:
        mesh = new Block(scale);
...
void SceneEditor::SetUniformsShaderEditor(H2M::RefH2M<ImaGEShader> shaderEditor, H2M::RefH2M<ImaGETexture> texture, SceneObject* sceneObject)
{
    shaderEditor->Bind();
...
void SceneEditor::SetUniformsShaderEditorPBR(H2M::RefH2M<ImaGEShader> shaderEditorPBR, H2M::RefH2M<ImaGETexture> texture, H2M::RefH2M<Material> material, SceneObject* sceneObject)
{
...
void SceneEditor::SetUniformsShaderSkinning(H2M::RefH2M<ImaGEShader> shaderSkinning, SceneObject* sceneObject, float runningTime)
{
...
void SceneEditor::SetUniformsShaderHybridAnimPBR(H2M::RefH2M<ImaGEShader> shaderHybridAnimPBR, H2M::RefH2M<ImaGETexture> texture, SceneObject* sceneObject, float runningTime)
{
    RendererBasic::DisableCulling();
...
void SceneEditor::SetUniformsShaderWater(H2M::RefH2M<ImaGEShader> shaderWater, SceneObject* sceneObject, glm::mat4& projectionMatrix)
{
...
void SceneEditor::RenderLightSources(H2M::RefH2M<ImaGEShader> shaderGizmo)
{
    shaderGizmo->Bind();
...
void SceneEditor::RenderSkybox(H2M::RefH2M<ImaGEShader> shaderBackground)
{
    // m_BlurEffect->Render();
...
void SceneEditor::RenderLineElements(H2M::RefH2M<ImaGEShader> shaderBasic, glm::mat4 projectionMatrix)
{
...
void SceneEditor::RenderFramebufferTextures(H2M::RefH2M<ImaGEShader> shaderEditor)
{
...
void SceneEditor::RenderGlassObjects(H2M::RefH2M<ImaGEShader> shaderGlass)
{
...
void SceneEditor::Render(Window* mainWindow, glm::mat4 projectionMatrix, std::string passType,
    std::map<std::string, H2M::RefH2M<ImaGEShader>> shaders, std::map<std::string, int> uniforms)
{
    ...
    for (auto& object : m_SceneObjects)
    {
        ...
        H2M::RefH2M<ImaGETexture> texture = ResourceManager::HotLoadTexture(object->textureName);
