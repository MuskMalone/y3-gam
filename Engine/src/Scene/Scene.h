...
class Scene : public H2M::SceneH2M
{
public:
	Scene();
	virtual ~Scene();
	...
	virtual void Render(Window* mainWindow, glm::mat4 projectionMatrix, std::string passType,
		std::map<std::string, H2M::RefH2M<ImaGEShader>> shaders, std::map<std::string, int> uniforms) = 0;
	...
	std::map<std::string, H2M::RefH2M<ImaGETexture>> GetTextures() const { return textures; }
...
protected:
	...
	std::map<std::string, H2M::RefH2M<ImaGETexture>> textures;
...
private:
	...
	std::map<std::string, H2M::RefH2M<ImaGEShader>> shaders;
