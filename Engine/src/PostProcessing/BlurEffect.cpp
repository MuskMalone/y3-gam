...
void BlurEffect::SetupShaders()
{
	if (!m_ShaderHorizontalBlur) {
		m_ShaderHorizontalBlur = ImaGEShader::Create("Shaders/ThinMatrix/blur_horizontal.vs", "Shaders/ThinMatrix/blur.fs");
	...
	if (!m_ShaderVerticalBlur) {
		m_ShaderVerticalBlur = ImaGEShader::Create("Shaders/ThinMatrix/blur_vertical.vs", "Shaders/ThinMatrix/blur.fs");
...
void BlurEffect::HorizontalBlurSetup(int width, int height)
{
	...
	m_HorizontalFBO = ImaGEFramebuffer::Create(m_Width, m_Height);
...
void BlurEffect::VerticalBlurSetup(int width, int height)
{
	...
	m_VerticalFBO = ImaGEFramebuffer::Create(m_Width, m_Height);
...
H2M::RefH2M<FramebufferTexture> BlurEffect::GetHorizontalOutputTexture()
{
	return m_HorizontalFBO->GetTextureAttachmentColor();
}

H2M::RefH2M<FramebufferTexture> BlurEffect::GetVerticalOutputTexture()
{
