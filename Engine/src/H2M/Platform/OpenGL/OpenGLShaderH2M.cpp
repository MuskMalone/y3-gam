...
#define PRINT_SHADERS 1
	...
	void OpenGLShaderH2M::Reflect(std::vector<uint32_t>& data)
	{
		...
		IMAGE_CORE_TRACE("OpenGLShaderH2M::Reflect - {0}", m_AssetPath);
		IMAGE_CORE_TRACE("   {0} Uniform Buffers", res.uniform_buffers.size());
		IMAGE_CORE_TRACE("   {0} Resources", res.sampled_images.size());
		...
		for (const spirv_cross::Resource& resource : res.uniform_buffers)
		{
			...
			if (s_UniformBuffers.find(bindingPoint) == s_UniformBuffers.end())
			{
				...
				IMAGE_CORE_TRACE("Created Uniform Buffer at binding point {0} with name '{1}', size is {2} bytes", buffer.BindingPoint, buffer.Name, buffer.Size);
			...
			else
			{
				...
				if (bufferSize > buffer.Size) // Resize buffer if needed
				{
					...
					IMAGE_CORE_TRACE("Resized Uniform Buffer at binding point {0} with name '{1}', size is {2} bytes", buffer.BindingPoint, buffer.Name, buffer.Size);
				}
			}
		}
