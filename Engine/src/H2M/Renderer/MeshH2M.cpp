...
#define MESH_DEBUG_LOG 1
#if MESH_DEBUG_LOG
	#define HZ_MESH_LOG(...) IMAGE_CORE_TRACE(__VA_ARGS__)
#else
	#define HZ_MESH_LOG(...)
#endif
	...
	MeshH2M::MeshH2M(const std::string& filename, RefH2M<ImaGEShader> shader, RefH2M<MaterialH2M> material, bool isAnimated)
	...
	void MeshH2M::Create()
	{
		...
		if (!m_MeshShader)
		{
			/**** BEGIN ImaGEShader the new API ****/
			if (H2M::RendererAPI_H2M::Current() == H2M::RendererAPITypeH2M::OpenGL)
			{
				ImaGEShaderSpecification imaGEShaderSpecificationStatic;
				imaGEShaderSpecificationStatic.ShaderType = ImaGEShaderSpecification::ShaderType::ImaGEShader;
				imaGEShaderSpecificationStatic.VertexShaderPath = "Shaders/Hazel/HazelPBR_Static.vs";
				imaGEShaderSpecificationStatic.FragmentShaderPath = "Shaders/Hazel/HazelPBR.fs";
				// imaGEShaderSpecificationStatic.FragmentShaderPath = "Shaders/Hazel/HazelPBR_Static_CSM.fs"; // fragment shader with Cascaded Shadow Maps
				imaGEShaderSpecificationStatic.ForceCompile = false;

				ImaGEShaderSpecification imaGEShaderSpecificationAnim;
				imaGEShaderSpecificationAnim.ShaderType = ImaGEShaderSpecification::ShaderType::ImaGEShader;
				imaGEShaderSpecificationAnim.VertexShaderPath = "Shaders/Hazel/HazelPBR_Anim.vs";
				imaGEShaderSpecificationAnim.FragmentShaderPath = "Shaders/Hazel/HazelPBR.fs";
				imaGEShaderSpecificationAnim.ForceCompile = false;

				m_MeshShader = m_IsAnimated ? ImaGEShader::Create(imaGEShaderSpecificationAnim) : ImaGEShader::Create(imaGEShaderSpecificationStatic);
			}
			//else if (H2M::RendererAPI_H2M::Current() == H2M::RendererAPITypeH2M::Vulkan)
			//{
			//	ImaGEShaderSpecification imaGEShaderSpecificationHazelVulkan;
			//	imaGEShaderSpecificationHazelVulkan.ShaderType = ImaGEShaderSpecification::ShaderType::HazelShader;
			//	imaGEShaderSpecificationHazelVulkan.HazelShaderPath = "Resources/Shaders/HazelPBR_Static.glsl";
			//	imaGEShaderSpecificationHazelVulkan.ForceCompile = true;

			//	m_MeshShader = ImaGEShader::Create(imaGEShaderSpecificationHazelVulkan);
			//}
			//else if (H2M::RendererAPI_H2M::Current() == H2M::RendererAPITypeH2M::DX11)
			//{
			//	ImaGEShaderSpecification imaGEShaderSpecificationHazelDX11;
			//	imaGEShaderSpecificationHazelDX11.ShaderType = ImaGEShaderSpecification::ShaderType::DX11Shader;
			//	imaGEShaderSpecificationHazelDX11.VertexShaderPath = "Shaders/HLSL/BasicVertexShader.hlsl";
			//	imaGEShaderSpecificationHazelDX11.PixelShaderPath = "Shaders/HLSL/BasicPixelShader.hlsl";
			//	imaGEShaderSpecificationHazelDX11.ForceCompile = false;
			//	m_MeshShader = ResourceManager::CreateOrLoadShader(imaGEShaderSpecificationHazelDX11);
			//}
			/**** END ImaGEShader the new API ****/
		}
		...
		auto shader = m_MeshShader; // ImaGEShader::Create("Resources/Shaders/VulkanWeekMesh.glsl", true);
		...
		//		// // IMAGE_CORE_WARN("Updating {0} descriptor sets", s_WriteDescriptorSets.size());
	...
	void SubmeshH2M::Render(RefH2M<MeshH2M> parentMesh, RefH2M<ImaGEShader> shader, const glm::mat4& entityTransform, uint32_t samplerSlot,
	...
	void SubmeshH2M::RenderOutline(RefH2M<MeshH2M> parentMesh, RefH2M<ImaGEShader> shader, const glm::mat4& entityTransform, EntityH2M entity)
	{
		parentMesh->GetVertexBuffer()->Bind();
		parentMesh->GetPipeline()->Bind();
		parentMesh->GetIndexBuffer()->Bind();
