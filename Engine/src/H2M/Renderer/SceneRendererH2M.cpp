...
namespace H2M
{
	...
	struct SceneRendererDataH2M
	{
		...
		RefH2M<ImaGEShader> BloomBlurShader;
		RefH2M<ImaGEShader> BloomBlendShader;
		...
		RefH2M<ImaGEShader> ShadowMapShader, ShadowMapAnimShader;
...
#if 0
		s_Data.CompositeShader = ImaGEShader::Create("Resources/Shaders/SceneComposite.glsl");
		s_Data.BloomBlurShader = ImaGEShader::Create("Resources/Shaders/BloomBlur.glsl");
		s_Data.BloomBlendShader = ImaGEShader::Create("Resources/Shaders/BloomBlend.glsl");
#endif
	...
	void SceneRendererH2M::OnImGuiRender()
	{
		...
		if (UI::BeginTreeNode("Visualization"))
		{
			...
			//UI::PropertySlider("Selected Draw", OpenGLRendererH2M::GetSelectedDrawCall(), -1, maxDrawCall);
